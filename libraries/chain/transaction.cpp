...
digest_type packed_transaction::prunable_data_type::digest() const {
  return fc::visit( [](const auto& obj) { return prunable_digest(obj); }, prunable_data );
}

packed_transaction::prunable_data_type packed_transaction::prunable_data_type::prune_all() const {
  return fc::visit([](const auto& obj) -> packed_transaction::prunable_data_type { return {none{prunable_digest(obj)}}; }, prunable_data);
}
...
const vector<bytes>* packed_transaction::get_context_free_data()const {
   if( fc::holds_alternative<prunable_data_type::full>(prunable_data.prunable_data) ) {
      return &fc::get<prunable_data_type::full>(prunable_data.prunable_data).context_free_segments;
   } else if( fc::holds_alternative<prunable_data_type::full_legacy>(prunable_data.prunable_data) ) {
      return &fc::get<prunable_data_type::full_legacy>(prunable_data.prunable_data).context_free_segments;
   } else {
      return nullptr;
   }
...
void packed_transaction::reflector_init()
{
   ...
   trx_id = unpacked_trx.id();
   if( fc::holds_alternative<prunable_data_type::full_legacy>(prunable_data.prunable_data) ) {
